...
本指南定义 DiceFrame 插件的通用包结构、manifest 标准、配置规则和各类插件的扩展边界。当前真正可用的是聊天桥接、内容包、安全主题变量，以及地图包中的地点/素材注册。导入导出、Provider 和工具类目前只有类型占位，没有对应业务运行时。

**重要：只有“当前支持状态”标为“已支持”或“部分支持”的能力，照本文开发后才会真实生效。标为“预留”的类型在开发目录中只能被识别和展示，不会接入导入导出、Provider 或工具流程，插件商店也不允许安装。插件 README 必须如实说明当前作用。**
...
| 工具 | `tool` | 预留：尚无工具调用运行时，商店不允许安装 |

`content-pack`、`theme`、`map-pack` 是声明型插件，可以没有后台进程。`channel-adapter` 需要 `entrypoint`，由宿主作为独立进程托管。预留类型即使可以在开发目录中被识别，也不代表已经具有对应业务能力。
...
适用于批量校验、备份、转换、生成器等辅助能力。当前仅保留类型定义，统一工具运行时后续实现，商店不允许安装此类型。
